...
const register = asyncErrorWrapper(async (req, res, next) => {
    ...
    //Token elde etmek için aşağıdaki kod blopunu kullandık pnun yerine helper/authorization kullandık
    ...
    //Express Async Handler Kullanmazsak Aşağıdaki Gibi Ama Express Async Handler ile try-catch kısmını kendi hallediyor
    // try {
    //     const user = await User.create({
    //         name,
    //         email,
...
const resetPassword = asyncErrorWrapper(async (req, res, next) => {
    ...
    if (!user) {
        return next(new CustomError("Invalid Token or Session Experied", 404))
    }
